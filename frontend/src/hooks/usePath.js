...
import { 
  ...
  fetchOpenRouteServiceFallbackRoute,
  createDirectPath 
...
const usePath = (obstacles = []) => {
  ...
  const fallbackToDirectPath = useCallback(async (start, end) => {
    try {
      console.log("Attempting to create a path using OpenRouteService fallback method");
      
      // Try simplified OpenRouteService route first
      const data = await fetchOpenRouteServiceFallbackRoute(start, end);
    ...
    // If OpenRouteService fails completely, create a simple direct path
    try {
  ...
  const findPath = useCallback(async (start, end) => {
    try {
      ...
      const data = await fetchOpenRouteServiceRoute(start, end);
  ...
  const filterSimilarPaths = (paths) => {
    ...
    return ensureUniquePaths(uniquePaths);
  };
